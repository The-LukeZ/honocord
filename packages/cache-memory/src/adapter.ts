...
export class MemoryCacheAdapter extends BaseCacheAdapter {
  ...
  constructor(private options: Pick<CacheOptions, "ttl"> = {}) {
    super();
  }
  ...
  private startCleanup() {
    if (!!this.options.ttl) {
      this.cleanupInterval = setInterval(() => this.cleanup(), (this.options.ttl ?? 60) * 1000);
    }
  }
  ...
  async set<T>(key: string, value: T, ttlMs?: number): Promise<void> {
    ...
    if (!this.cleanupInterval) this.startCleanup(); // Start cleanup if not already running
  }
