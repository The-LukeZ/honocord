...
<Aside type="tip">
On Cloudflare Workers, webhook events are processed asynchronously using `waitUntil`. This means:
- Discord receives an immediate `200 OK` response
- Your handler code runs in the background
- The worker's lifetime is extended to complete processing
- Prevents timeouts for slow operations (API calls, database writes)
</Aside>
...
<Aside type="caution">
Worker mode handlers (`forWorker: true`) cannot use `fetch()` or `getApp()` because they don't return Response objects. Use them with Honocord's `webhookHandler` instead.
</Aside>
