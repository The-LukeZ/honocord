...
A common pattern is to show a truncated preview in an embed while attaching the full data as a downloadable file.
...
await ctx.editReply({
  ...
  files: [
    new AttachmentBuilder(Buffer.from(json), {
      name: `${userData.username.replace(" ", "-").toLowerCase()}.json`,
      description: "Full user data as JSON",
    }),
  ],
});
````
