...
The type parameter `T` is typically inferred from the `eventType` constructor argument. If you need to specify custom `Env` or `Variables` types, you must explicitly specify `T` first: `new WebhookEventHandler<ApplicationWebhookEventType.YourEvent, YourEnv, YourVariables>(...)`.
:::
