...
- `Env` (default: `any`) - Environment bindings type, must include `DISCORD_PUBLIC_KEY?: string`
- `Variables` (default: `{}`) - Additional context variables
- `T` - Discord webhook event type from `ApplicationWebhookEventType`
- `Data` - Typed webhook event data payload

## Constructor

```ts
new WebhookEventHandler<ForWorker, Env, Variables, T, Data>(
  eventType: T,
  forWorker?: ForWorker
)
```
