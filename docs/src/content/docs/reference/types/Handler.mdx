...
This union ensures that only valid handler instances are passed to the bot. Each member of the union corresponds to a specific type of Discord interaction (Slash Commands, User context menus, Message context menus, Buttons, Select Menus, and Modals).
